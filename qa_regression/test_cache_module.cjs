...
async function runTests() {
  ...
  const rootCachePath = path.resolve('_gemini_cache.json');
  const rootExistedBefore = fs.existsSync(rootCachePath);
  const originalRootBytes = rootExistedBefore ? fs.readFileSync(rootCachePath) : null;
  ...
  try {
    ...
    // ── ALWAYS Clean up and restore root _gemini_cache.json ────────────
    if (fs.existsSync(testDir)) {
      fs.rmSync(testDir, { recursive: true, force: true });
    }

    if (rootExistedBefore) {
      fs.writeFileSync(rootCachePath, originalRootBytes);
    } else if (fs.existsSync(rootCachePath)) {
      fs.rmSync(rootCachePath, { force: true });
    }

    // Assert pre-test root cache state is restored byte-for-byte
    if (rootExistedBefore) {
      const currentBytes = fs.readFileSync(rootCachePath);
      const isIdentical = Buffer.compare(currentBytes, originalRootBytes) === 0;
      if (isIdentical) {
        console.log('✓ ISOLATION ASSERTION PASSED: Repo-root _gemini_cache.json restored byte-for-byte.');
      } else {
        console.error('❌ ISOLATION ASSERTION FAILED: Root cache content mismatch after test!');
        allPassed = false;
      }
    } else {
      const stillExists = fs.existsSync(rootCachePath);
      if (!stillExists) {
        console.log('✓ ISOLATION ASSERTION PASSED: Test-created repo-root _gemini_cache.json cleanly removed.');
      } else {
        console.error('❌ ISOLATION ASSERTION FAILED: Root cache was not removed after test!');
        allPassed = false;
      }
    }
  }
